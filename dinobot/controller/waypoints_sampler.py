...
def sample_pose_on_sphere(distance_range=(0, 1), phi_range=(0, np.pi/2)):
    """
...
class PickAndPlaceWaypointSampler:
    ...
    def sample_waypoints(self, object_ids):
        ...
        # Pre-grasp pose
        pre_grasp_pose = sample_pose_on_sphere(distance_range=(0.15, 0.3), phi_range=(0, 0.3))
        rel_waypoints.append(np.concatenate([np.array(first_obj_id).reshape(-1,), np.array(pre_grasp_pose)]))
        # Approach first object
        grasp_pose = sample_pose_on_sphere(distance_range=(0.05, 0.2))
        ...
        # Pre-place pose
        pre_place_pose = sample_pose_on_sphere(distance_range=(0.15, 0.3), phi_range=(0, 0.3))
        rel_waypoints.append(np.concatenate([np.array(second_obj_id).reshape(-1,), np.array(pre_place_pose)]))
        # Approach second object
        place_pose = sample_pose_on_sphere(distance_range=(0.05, 0.2))
...
class MoveToTargetWaypointSampler:
    ...
    def sample_waypoints(self, object_ids):
        ...
        rel_waypoints = []
        sampled_pose = sample_pose_on_sphere(distance_range=(0.05, 0.2))
...
class PickWaypointSampler:
    ...
    def sample_waypoints(self, object_ids):
        ...
        rel_waypoints.append(np.concatenate([np.array(first_obj_id).reshape(-1,), GRASP_CODE]))
        # Post-grasp pose
        post_grasp_pose = sample_pose_on_sphere(distance_range=(0.15, 0.3), phi_range=(0, 0.3))
