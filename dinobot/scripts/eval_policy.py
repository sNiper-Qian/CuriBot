...
def main(dataset_config: DatasetConfig, 
    ...
    for data in eval_loader:
        ...
        prompt_frames_xz = []
        prompt_frames_yz = []
        for i in range(prompt_images_xy.shape[0]):
        ...
        # save video as mp4
        frames_to_video(prompt_frames_xy, output_path=f"videos/{current_time}/prompt_xy_{trials}.mp4", fps=3)
        frames_to_video(prompt_frames_xz, output_path=f"videos/{current_time}/prompt_xz_{trials}.mp4", fps=3)
        frames_to_video(prompt_frames_yz, output_path=f"videos/{current_time}/prompt_yz_{trials}.mp4", fps=3)
        ...
        with torch.no_grad():
            ...
            # save video as mp4
            frames_to_video(frames_xy, output_path=f"videos/{current_time}/output_xy_{trials}.mp4", fps=3)
            frames_to_video(frames_xz, output_path=f"videos/{current_time}/output_xz_{trials}.mp4", fps=3)
            frames_to_video(frames_yz, output_path=f"videos/{current_time}/output_yz_{trials}.mp4", fps=3)
