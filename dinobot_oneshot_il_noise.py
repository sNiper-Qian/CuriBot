...
stepsize = 1 / 500
robot_origin = SimpleEnv(render=True)
# robot_origin.reset()
...
def project_to_3d(points, depth, intrinsics):
    ...
    depth_values = np.array([depth[int(x), int(y)] for x, y in points])
    points_world = robot.depth_to_world(depth)
    # o3d.visualization.draw_geometries([points_world])
    
    # print(f'points_world: {points_world[19596][2]}')
    ...
    for i in range(len(points)):
        x, y = points[i]
        depth_dino.append(points_world[x * width + y][2])
        # print(f'points_world: {points_world[x * width + y]}')
        target_points_world.append(points_world[x * width + y].tolist())
    target_points_world = np.array(target_points_world)
    # print(f'depth_dino: {depth_dino}')
    ...
    # pixels = np.stack([normalized_x, normalized_y, depth_dino, np.ones_like(depth_dino)], axis=1)
    ...
    visualizer.save_visualization_to_file(points_world, f'point_cloud/{task}_pointcloud{points[0]}.html')
    visualizer.save_visualization_to_file(target_points_world, f'point_cloud/{task}_pointcloud{points[0]}_dino_point.html')
...
def move_point_to_boundary(point, boundary_coords):
    """
    Move a point to the nearest location on the boundary if it's outside
    :param point: Tuple (x, y)
    :param boundary_coords: List of [x, y] coordinates defining the polygon
    :return: New point coordinates (on the boundary if originally outside)
    """
    # Convert boundary coordinates to numpy array
    boundary_array = np.array(boundary_coords, dtype=np.int32)
    
    # Check if point is inside the boundary
    distance = cv2.pointPolygonTest(boundary_array, point, measureDist=True)
    
    if distance >= 0:
        # Point is already inside or on the boundary
        return point
    else:
        ...
        for i in range(len(boundary_coords)):
            ...
            t = np.dot(point_vec, segment_vec) / (segment_length ** 2)
            t = max(0, min(1, t))  # Clamp to segment
            projection = np.array(start) + t * segment_vec
            
            # Calculate distance to projection
            dist = np.linalg.norm(np.array(point) - projection)
...
if __name__ == "__main__":
    ...
    robot = SimpleEnv(Test_env=True, render=True)
    ...
    while 1:
        ...
        while error > ERR_THRESHOLD:
            ...
            #     fg1.savefig(f"fg1.pdf", dpi=224, bbox_inches='tight')
            #     fg2.savefig(f"fg2.pdf", dpi=224, bbox_inches='tight')
            #     plt.close()
                # plt.show()
            ...
            # points1 = [(88, 108), (76, 116), (88, 116)]
            # # points2 = [(128, 100), (84, 108), (116, 132)]
            # points1 = [(88, 116), (88, 112), (76, 116), (88, 120)]
            # points2 = [(68, 92), (64, 84), (24, 84), (68, 96)]
            points1 = [(88, 108), (92, 108), (76, 116), (76, 108)]
            points2 = [(56, 108), (60, 108), (44, 116), (44, 108)]
            ...
            def add_noise(coord, noise_range):
                return coord + np.random.uniform(-noise_range, noise_range)
            ...
            print(f'boundry: {boundary_coords}')

            points2_noisy = [(add_noise(x, std_dev), add_noise(y, std_dev)) for x, y in points2]
            points2_noisy = [(int(x), int(y)) for x, y in points2_noisy]
            print(f'noisy_points2: {points2_noisy}')

            adjusted_points2 = [move_point_to_boundary(point, boundary_coords) for point in points2_noisy]
            print(f'adjusted_points2: {adjusted_points2}')
